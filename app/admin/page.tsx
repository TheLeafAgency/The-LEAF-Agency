...
export default function AdminPage() {
  ...
  const expandedRequests = useMemo(() => {
    ...
    if (expandedStat === "new") return sortProjects(requests.filter((item) => (!item.progressStatuses?.length || (item.progressStatuses.length === 1 && item.progressStatuses[0] === "Untouched")) && !["Completed", "Failed", "Declined", "Urgent"].includes(displayStatus(item.status))));
